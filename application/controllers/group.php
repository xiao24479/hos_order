...
class Group extends CI_Controller{
    ...
    public function set_group(){
	    ...
	    if($rank_id > 0 && $id >0 ){
	    	...
	    	$this->load->view('group/group_set', $data);
	    }
    }
    ...
    public function add_group(){
        ...
        if($id > 0){
        	...
        	$this->db->trans_complete(); 
        	***/
            $link[0]=array('href'=>'?c=group&m=group_list','text'=>'组列表');
    ...
    public function admin_list_ajax()
    {
    	...
    	if(empty($keshi_id)){
    		$sql="select a.admin_id,a.admin_name from ".$this->common->table('rank_power')." as rp,".$this->common->table('admin')." as a,".$this->common->table('rank')." as r where rp.hos_id = ".$hos_id."  and rp.rank_id  = r.rank_id and r.rank_id = a.rank_id  and (a.admin_group =0 or a.admin_group = '' or a.admin_group is null)  and  a.is_pass = 1 ORDER  BY CONVERT( a.admin_username
USING gbk )  COLLATE gbk_chinese_ci ASC ";
    	}else{
    		$sql="select a.admin_id,a.admin_name from ".$this->common->table('rank_power')." as rp,".$this->common->table('admin')." as a,".$this->common->table('rank')." as r where rp.hos_id = ".$hos_id." and rp.keshi_id = ".$keshi_id." and rp.rank_id  = r.rank_id and r.rank_id = a.rank_id  and (a.admin_group =0 or a.admin_group = '' or a.admin_group is null)  and  a.is_pass = 1    ORDER  BY CONVERT( a.admin_username
USING gbk )  COLLATE gbk_chinese_ci ASC ";
    	}
    ...
    public function group_add(){
       $data=array(); 
       $data=$this->common->config('group_add');
       $data['top'] = $this->load->view('top', $data, true);
       $data['themes_color_select'] = $this->load->view('themes_color_select', '', true);
       $data['sider_menu'] = $this->load->view('sider_menu', $data,true);
       ...
       $sql="select * from ".$this->common->table('hospital')."  ORDER BY CONVERT(hos_name USING gbk) asc";
       $data['hospital_list']=$this->common->getAll($sql); 
    ...
    public function group_list(){
        ...
        $sql="select * from ".$this->common->table('user_groups')."    ORDER  BY CONVERT( name USING gbk )  COLLATE gbk_chinese_ci ASC";
   ...
   //编辑系统组页面
    public function update_group(){
		$data=array(); 
        $data=$this->common->config('group_update');
        ...
        $data['themes_color_select'] = $this->load->view('themes_color_select', '', true);
        $data['sider_menu'] = $this->load->view('sider_menu', $data,true);
        $this->load->view('group/group_update', $data);  
    }
    //更新系统消息
    public function group_update(){
		...
		if($id == 0){
			...
			$parent_id   = $_REQUEST['parent_id'];
			$group=array( 'name'   =>$name,'edit_time'   =>time(),'parent_id'=>$parent_id );
			$this->db->trans_start();
			$this->db->where('id', $id);
    ...
    public function del_group(){
        ...
        if($bol){
            ...
            $link[0]=array('href'=>'?c=group&m=group_list','text'=>'组列表');
            $this->common->msg('删除成功！',1,$link);
        }else{
             $this->common->msg('删除失败，返回！',0);
        }
    }
}