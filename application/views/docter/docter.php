...
<!--                            <div class="widget-title">
  ...
  <tr>
	...
	<th width="250"><?php echo $this->lang->line('action'); ?></th>
  </tr>
  </thead>
  <tbody>
  <?php foreach($hos_list as $item): ?>
  <tr>
	<td><a href="javascript:;" onClick="get_keshi(this, <?php echo $item['hos_id'];?>)"><i class="icon-plus"></i> <?php echo $item['hos_name']?></a></td>
	<td><a href="<?php echo $item['hos_website']; ?>" target="_blank"><?php echo $item['hos_website']; ?></a></td>
	<td>
	<button class="btn btn-primary" onClick="go_url('?c=system&m=hospital_info&hos_id=<?php echo $item['hos_id']?>')"><i class="icon-pencil"></i></button>&nbsp;&nbsp;
	<button class="btn btn-primary" onClick="go_url('?c=system&amp;m=docter_info&hos_id=<?php echo $item['hos_id']?>')"><i class="icon-plus-sign"></i></button>
	</td>
  </tr>
  <?php endforeach; ?>
  </tbody>
...
<script>
function get_doc(obj, hos_id,keshi_id)
{
	if($(obj).children("i").attr("class") == "icon-minus")
	{
		$(obj).children("i").attr("class", "icon-refresh icon-spin");
		$(".doc_" + hos_id).slideUp(100, function(){ $(obj).children("i").attr("class", "icon-plus"); });
	}
	else
	{
