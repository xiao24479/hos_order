...
<input type="hidden" value="docter_index" name="m" />
  ...
  if(!empty($order_list)){
    ...
    <div id="pat_<?php echo $item['order_id']; ?>">
        ...
        姓名：<?php if(!empty($item['zampo'])):?><del><?php endif;?><font style="color:#ff6060;font-size:14px;" id="pat_name_<?php echo $item['order_id']; ?>"><?php echo $item['pat_name']?></font>（<?php echo $item['pat_sex']; ?>、<?php echo $item['pat_age']?>岁） <a href="#kefu_talk" title="对话记录" role="button" data-toggle="modal" onClick="kefu_talk('<?php echo $item['order_id']?>');_czc.push(['_trackEvent', '预约相关', '<?php echo $admin['name']; ?>', '对话记录','','']);"><img src="static/img/talk.png" width="16px;"/></a> <br />
