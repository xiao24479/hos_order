...
<body class="fixed-top" style="width:1100px;overflow:hidden; background-color:#FFF;">
...
<div class="control-group">
	...
	<div class="controls">
		...
		<input type="text" value="<?php echo $info['order_no'];?>" data-trigger="hover" class="input-large" name="order_no" id="order_no" readonly />&nbsp;<button type="button" id="yunzhou_btn" class="btn"> 孕周计算 </button>
...
<div class="span6 order_from">
	...
	<div class="control-group order_from">
		...
		<div class="controls">
		   ...
		   <select name="hos_id" id="hos_id" style="width:224px;">
...
<div class="span6 order_from">
	...
	<div class="control-group order_from">
		...
		<div class="controls">
		   ...
		   <select name="jibing_parent_id" id="jibing_parent_id" style="width:224px;">
...
<div class="span5 order_from">
	...
	<div class="control-group order_from">
		...
		<div class="controls">
		    ...
		    <?php   if(isset($order_edit_consultants)){?>
            ...
            <select name="admin_id" id="admin_id">
             ...
             <?php }else { ?>
                 <select name="admin_id" id="admin_id">
					    <option value="<?php echo $info['admin_id'];?>" selected  ><?php echo $info['admin_name'];?></option>
                   </select>
			  <?php }?>
...
<div class="control-group order_from">
	...
	<div class="controls">
	   ...
	   <select name="from_parent_id" id="from_parent_id" style="width:224px;">
	   ...
	   <input type="text" name="from_value" id="from_value" value="<?php echo $info['from_value'];?>" class="input-xlarge" readonly  style="width:315px;"/>
...
<div class="span6 order_from">
	...
	<div class="control-group order_from">
		...
		<div class="controls">
               <?php if(!isset($order_edit_person_info)){ ?>
               <input type="hidden" value="<?php echo $info['pat_name'];?>" id="patient_name" name="patient_name" />
               <?php echo $info['pat_name'];?>
			   <?php }else{ ?>
...
<div class="span6 order_from">
	...
	<div class="control-group order_from">
		...
		<div class="controls" id="phone_area">
	    ...
	    <?php  }else if(!isset($order_edit_person_info)){ ?>
               ...
               <?php $l_admin_action  = explode(',',$_COOKIE['l_admin_action']); ?>
				<?php if( ($phone_show != 1) && ($info['admin_id'] != $_COOKIE['l_admin_id']) && ($_COOKIE['l_admin_action'] != 'all') && !in_array(198, $l_admin_action)):?>
               			<span id="disabled_phone"><?php echo substr($info['pat_phone'],0, -4)."****"; if(!empty($info['pat_phone1'])){echo "/" . substr($info['pat_phone1'],0, -4)."****";}?></span><a class="btn btn-default icon-eye-open eye_phone_<?php echo $info['pat_id']; ?>" onclick="show_phone(this,<?php echo $info['order_id']; ?>,<?php echo $info['pat_id']; ?>,2);"></a>
               	<?php else: ?>
					<span><?php echo $info['pat_phone']; if(!empty($info['pat_phone1'])){echo "/" . $info['pat_phone1'];}?></span>
               	<?php endif; ?>

		   <?php }else{ ?>
				<?php $l_admin_action  = explode(',',$_COOKIE['l_admin_action']); ?>
				<?php if( ($phone_show != 1) && ($info['admin_id'] != $_COOKIE['l_admin_id']) && ($_COOKIE['l_admin_action'] != 'all') && !in_array(198, $l_admin_action)):?>
					<input type="text" value="<?php echo substr($info['pat_phone'],0, -4)."****"; if(!empty($info['pat_phone1'])){echo "/" . substr($info['pat_phone1'],0, -4)."****";}?>" id="disabled_phone" disabled>
					<a class="btn btn-default icon-eye-open eye_phone_<?php echo $info['pat_id']; ?>" onclick="show_phone(this,<?php echo $info['order_id']; ?>,<?php echo $info['pat_id']; ?>,1);"></a>

					<input type="hidden" name="patient_phone" value="<?php echo $info['pat_phone']; if(!empty($info['pat_phone1'])){echo "/" . $info['pat_phone1'];}?>" id="phone_hidden">
		...
		<div class="controls">
			...
			<div class="input-icon left">
			 ...
			 <?php if($info['keshi_id'] == 32 || $info['keshi_id'] == 1){?>
						...
						<input  type="text" name="order_time" class="input-large" value="<?php echo date("Y-m-d", $info['order_time']); ?>" id="order_time" style="width:90px;" />
						...
						<input  type="hidden" name="order_null_time" class="input-large" value="" id="order_null_time" style="width:90px;"/>
						...
						<input  type="hidden" name="order_time" class="input-large" value="<?php echo date("Y-m-d", time()); ?>" id="order_time" style="width:90px;" />

		                  <input  type="hidden" id="order_hidden_time" value="<?php echo date("Y-m-d", time()); ?>"/>

		                   <input  type="hidden" id="order_hidden_time_duan_j" value="<?php if($info['duan_confirm'] == 2){ echo $info['order_time_duan'];}else{ echo "12:00";}?>"/>

						<input  type="text" name="order_null_time" class="input-large" value="<?php echo $info['order_null_time']; ?>" id="order_null_time" style="width:90px;" />
						...
						<?php endif;?>
		                <color id="order_time_type_msg" style="color:red;"><!-- <?php if($yd_time != 0 && $_COOKIE['l_admin_id'] == $info['admin_id']){?>修改次数剩余:<?php echo $yd_sum;?>次;最大预到时间:<?php echo $yd_time;}?>--></color>
				<?php }else{?>
						<?php if(!empty($info['order_time'])):?>
	...
	<div class="control-group order_from">
		...
		<div class="controls">
          ...
          <div class="input-append" id="order_time_duan_b" style="margin-right:5px;float:left;<?php if($info['duan_confirm'] == 1){ echo "display:none;";}?>">
           ...
           <color id="order_time_duan_type_msg" style="color:red;"></color>
...
<div class="row-fluid order_from" >
	<label class="control-label">患者时间段</label>
	<div class="controls">
		<select name="hzsjd" id="hzsjd" <?php if( $_COOKIE['l_admin_id'] != 71 && $_COOKIE['l_admin_action'] != 'all' ):?> disabled <?php endif; ?>>
			<option value="0"><?php echo $this->lang->line('please_select');?></option>
			<option value="1" <?php if($info['hzsjd'] == 1){echo "selected";}?>>一年以内</option>
			<option value="2" <?php if($info['hzsjd'] == 2){echo "selected";}?>>一年以上</option>
		</select>
	</div>
</div>
...
<div class="control-group order_from">
	...
	<div class="controls">
        ...
        已怀孕<input  type="text" value=""  readonly="readonly" name="yunzhou" id="yunzhou" placeholder="孕周" style="width:24px; border: 0px; color: red; text-align: center; font-weight: bold;" />周，
        第<input type="text" value=""  readonly="readonly" name="yunzhoutian" id="yunzhoutian" placeholder="怀孕天数" style="width:24px; border: 0px; color: red; text-align: center; font-weight: bold;" />天，
        预产期是<input type="text" value=""  readonly="readonly" class="input-large" name="yuchanqi" id="yuchanqi" placeholder="预产期"  style="border: 0px;" />
...
?>
		...
		<textarea class="input-xxlarge " rows="2" name="remark" style="width:768px; border:1px solid #00F;"></textarea>
...
<div class="control-group order_from">
	...
	<div class="controls">
		...
		<textarea class="input-xxlarge " rows="3" name="sms_content" id="sms_content" style="width:768px;"></textarea>
...
<div class="control-group order_from">
	...
	<div class="description controls">
             <script id="editor_info" type="text/plain"><?php echo $con_content; ?></script>
     </div>
	<!-- 将商品详情赋值到此处 -->
    <textarea name="con_content" style="display: none;"  id="con_content" ><?php echo $con_content; ?></textarea>
...
<button type="reset" class="btn btn-inverse"><i class="icon-remove"></i> <?php echo $this->lang->line('reset'); ?> </button>
...
<div class="control-group order_from">
	...
	<div class="controls">
	   ...
	   <input type="text" name="from_value" id="from_value" value="" class="input-xlarge" readonly  style="width:315px;"/>
...
<div class="span6 order_from">
	<div class="control-group order_from">
		<label class="control-label">QQ</label>
		<div class="controls">
		<input type="text" value="" class="input-large" name="patient_qq" style="width:205px;" />
		</div>
	</div>
</div>
<div class="span5 order_from">
	<div class="control-group">
		<label class="control-label">微信</label>
		<div class="controls">
			<input type="text" value="" class="input-large" name="patient_weixin" style="width:205px;" />
	    </div>
	</div>
</div>
</div>
	...
	<div class="control-group">
		...
		<div class="controls">
			...
			<div class="input-icon left">
				...
				<input type="text" name="order_time" class="input-large" value="<?php echo date("Y-m-d"); ?>" id="order_time" style="width:90px;" />
...
<div class="control-group order_from" id="yunzhou_div" style="display:none;">
	...
	<div class="controls">
        ...
        已怀孕<input  type="text" value=""  readonly="readonly" name="yunzhou" id="yunzhou" placeholder="孕周" style="width:24px; border: 0px; color: red; text-align: center; font-weight: bold;" />周，
        第<input type="text" value="" name="yunzhoutian"  readonly="readonly" id="yunzhoutian" placeholder="怀孕天数" style="width:24px; border: 0px; color: red; text-align: center; font-weight: bold;" />天，
        预产期是<input type="text" value=""  readonly="readonly" class="input-large" name="yuchanqi" id="yuchanqi" placeholder="预产期"  style="border: 0px;" />
...
<div class="control-group order_from">
	...
	<div class="controls">
		...
		<input type="text" value="" class="input-xlarge" name="patient_address" style="width:305px;"/>
...
<div class="control-group order_from">
	<label class="control-label">商务通来源网址</label>
		<div class="controls">
		    <input type="text" name="laiyuanweb" id="laiyuanweb"   class="input-xlarge"  style="width:367px;"/>
		</div>
</div>
<div class="control-group order_from">
	<label class="control-label">关键字</label>
		<div class="controls">
		    <input type="text" name="guanjianzi" id="guanjianzi"   class="input-xlarge"  style="width:367px;"/>
		</div>
</div>
</div>
...
<div class="control-group order_from">
	...
	<div class="description controls">
             <script id="editor_info" type="text/plain"></script>
     </div>
	<!-- 将商品详情赋值到此处 -->
...
<div class="from_bottom">
    ...
    <span  class="btn btn-success" id="yuyue_btn">生成预约卡</span>
	<span onClick="show_card()" class="btn">隐藏/显示</span>
</div>
...
<?php endif;?>
	...
	$('#order_time').DatePicker({
		...
		onChange: function(formated, dates){
			...
			var hos_id = $('#hos_id').val();
			var old_time = $('#order_hidden_time').val() + ' 00:00:00';
			var old_time_timestamp = datetostr(old_time);
			var new_time = $('#order_time').val() + ' 00:00:00';
			var new_time_timestamp = datetostr(new_time);
			var todaytime = gettoday() + ' 00:00:00';
			var todaytimestamp = datetostr(todaytime);
			if ( (hos_id == 3 || hos_id == 6 || hos_id == 37 || hos_id == 42) && (todaytimestamp - new_time_timestamp) > 86400 ) {
				$("#order_time").val($("#order_hidden_time").val());
				$("#order_time_type_msg").html("最早只能改到当前时间的前一天！");
			}
	...
	$("#patient_phone").keyup(function(){
		//判断当前的医院 科室 电话号码是否变更 如果变更，才需要请求后台核实电话有效性
		if($("#hidden_check_hos_id").val() != $("#hidden_check_hos_id").val() || $("#hidden_check_keshi_id").val() != $("#keshi_id").val() ||  $.trim($("#defult_patient_phone").val()) != $.trim($("#patient_phone").val())){
...

{
	...
	$("#con_content").html(editor_info.getContent());
								...
								if (Date.parse(today) < Date.parse($("#order_time").val())) {
								  ...
								  if($("#keshi_id").val() == 87 || $("#keshi_id").val() == 88 || $("#keshi_id").val() == 89  || $("#keshi_id").val() == 85 || $("#keshi_id").val() == 86){
									  ...
									  if($("#from_id").val() == 200 || $("#from_id").val() == 201){
										  	...
										  	if(time1.getTime()>end_time){
											  $("#order_time_type_msg").html("不能超过7天哦！从今天开始计算。");
											  time_ok = 1;
									  		}
									  	}
								  }
								    ...
								    if (Date.parse(date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate()) > Date.parse($("#order_time").val())) {
									 ...
									 /**
										 ...
										 if($("#yd_time_check").val() != '' && $("#yd_time_check").val() != null && $("#yd_time_check").val() != 0){
												...
												判断仁爱妇科 不孕 是否通过验证条件
												1.以系统第一次录入的预到时间为准  往后延期renfi_fk_by_day_time天为 等待期，超过7天未到诊的 流入公海
												2.renfi_fk_by_day_time天之内 只有三次修改预到时间的机会。 添加时间  <= 修改时间    <=  renfi_fk_by_day_time 天预到时间
											    3.非当前数据 拥有者 可以在 当拥有者 3次机会使用完之后，去更改数据的 预到时间，条件是更改的时间必须大于 当前预到时间。同时将更改的时间 更新为 最大的预到限制时间
												4.变更咨询员会导致所有记时 限制的条件初始化
...
if(hos_id == 3 || hos_id == 6){
 ...
 //点击显示电话号码
 function show_phone(obj,order_id,pat_id,type)
 {
    $.post('?c=order&m=OrderShowPhone', {order_id: order_id, pat_id: pat_id}, function(data, textStatus, xhr) {
      if (data.code == 1) {
        ...
        $(".eye_phone_"+pat_id).hide(400).remove();

        if (type == 1) {
        	var __HTML = "<input type=\"text\" value=\""+data.data.phone+"\" data-trigger=\"hover\" data-placement=\"right\" data-content=\"【香港区号为：00852】【固定电话格式：075512345678】【手机号码前面不加0】\" data-original-title=\"号码输入规则\" class=\input-large popovers\" name=\"patient_phone\" id=\"patient_phone\" />";
        	$("#phone_hidden").remove();
        	$("#disabled_phone").remove();
        	$("#phone_area").append(__HTML);
        } else if (type ==2) {
        	var __HTML = data.data.phone;
        	$("#disabled_phone").text(__HTML);
        }
