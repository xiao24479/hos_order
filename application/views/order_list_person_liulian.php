...

{
	...
	window.location.href = '?c=order&m=order_list_person_liulian&type=mi';
...
<span>总留联人数：<font><?php echo $total_rows; ?></font></span>

<span>总预约人数：<font><?php echo $total_ok_rows; ?></font></span>
...
<input type="hidden" value="order_list_person_liulian" name="m" />
           ...
           <div id="dy_search" style=" position:fixed;width:100%;height:100px;;padding-top:10px;z-index: 3;"> 
               ...
               <div style="position:fixed;padding-top:22px;width:100%;height: 50px;line-height: 5px;font-family: '微软雅黑';color: #808080;font-size: 12px;background-color: #f7f7f7;">
		          ...
		          <input type="hidden" name="t" value="1">
                   ...
                   <span style="margin-left: 10px;"> 留联号：<input type="text" class="input_search" value="<?php echo $o_o; ?>" name="o_o" style="margin-top: 7px;width:70px;height:16px;font-size:12px; outline:none;"/> </span>
...
<div class="row-form" style="">
  ...
  if(!empty($order_list)){
    ...
    foreach($order_list as $item){
      ...
      <tr<?php if($i % 2){ echo " class='td1'";}?> style="height:90px;"  id="<?php echo $item['order_id']; ?>"  title="<?php echo date('H',strtotime($item['order_addtime'])); ?>">
...
<?php if(!empty($item['order_no_yy'])){?>
    ...
    <div id="pat_<?php echo $item['order_id']; ?>">
      ...
      预约单号：<a href="?c=order&m=order_list&t=1&o_o=<?php echo $item['order_no_yy'];?>"><span style="color:blue;"><?php echo $item['order_no_yy'];?></span></a>&nbsp;<?php  if(!empty($item['order_is_come'])){echo '<span style="color:red;">已到诊</span>'; }?><br/>转单时间:<?php echo date("Y-m-d H:i:s",$item['order_no_yy_time']);?><br/>
     ...
     姓名：<?php if(!empty($item['zampo'])):?><del><?php endif;?><font style="color:#ff6060;font-size:14px;font-weight:normal;" id="pat_name_<?php echo $item['order_id']; ?>"><?php echo $item['pat_name']?></font>（<?php echo $item['pat_sex']; ?>、<?php echo $item['pat_age']?>岁）
      ...
      <?php 
        ...
        <?php if(!empty($item['zampo'])):?></del><?php endif;?><br />
	...
	<td style="text-align:left;">
		...
		<?php echo $this->lang->line('order_time'); ?>：<font id="order_time_<?php echo $item['order_id']; ?>"><?php echo date("Y-m-d H:i:s",$item['order_time']); ?></font> <font style="color:#ff6060; font-weight:bold;" id="order_time_duan_<?php echo $item['order_id']; ?>"><?php if($item['order_time_duan']){ echo $item['order_time_duan'];}?></font><br />
		...
		<div style="display: none;"><?php echo $this->lang->line('come_time'); ?>：<span   id="come_time_<?php echo $item['order_id']; ?>"><?php if($item['come_time'] > 0){ echo date("Y-m-d H:i:s", $item['come_time']);}?></span></div><br />
...

{
...

	{
		...
		window.location.href = "http://www.renaidata.com/?c=order&m=order_list_person_liulian";
