...
<body class="fixed-top">
   ...
   <div id="main-content">
         ...
         <div class="container-fluid">
			...
			<input type="hidden" value="cpc_hour" name="m" />
			...
			<input type="hidden" value="<?php echo $site_info['site_id']; ?>" name="site_id" />
              <div class="row-fluid">
				<div class="filter_wrapper">
                    ...
                    <div class="filter selected"><?php echo $this->lang->line('cpc_hour'); ?></div>
              ...
              <div class="row-fluid">
				...
				<div class="widget-body">
				    <?php if(empty($cpc_hour)):?>暂无数据<?php else:?><div id="data" style="height:550px; width:98%; display:block;"></div><?php endif;?>
				</div>
			  </div>
			  </div>
            </div>
...
$(function() {
	...
	$(".title_select").change(function(){
		var value = $(this).val();
		var url = "?c=site&m=cpc_hour&site_id=<?php echo $site_info['site_id'];?>&date=<?php echo $start; ?> - <?php echo $end; ?>";
		window.location.href = url + "&type=" + value;
	});
});
<?php if(!empty($cpc_hour)):?>
    require.config({
        paths:{ 
				...
				xAxis : [
					{
						...
						data : [<?php $i = 1; foreach($cpc_hour['hour_hour'] as $val){ if($i > 1){ echo ",";} echo "'" . $val . "'"; $i ++;}?>]
					},
					{
						axisLabel : {
							formatter: '{value} 点'
				...
				series : [
					{
						name: '展现量',
						type: 'bar',
						data:[<?php $i = 1; foreach($cpc_hour['shows'] as $val){ if($i > 1){ echo ",";} echo $val; $i ++;}?>]
					},
					{
						name: '点击量',
						type: 'bar',
						data: [<?php $i = 1; foreach($cpc_hour['clicks'] as $val){ if($i > 1){ echo ",";} echo $val; $i ++;}?>]
					},
					{
						name: '点击率',
						type: 'line',
						yAxisIndex: 1,
						data: [<?php $i = 1; foreach($cpc_hour['click_lv'] as $val){ if($i > 1){ echo ",";} echo $val; $i ++;}?>]
					},
					{
						name:'消费',
						type: 'bar',
						data: [<?php $i = 1; foreach($cpc_hour['cost'] as $val){ if($i > 1){ echo ",";} echo $val; $i ++;}?>]
					},
					{
						name: '平均点击价格',
						type: 'bar',
						data: [<?php $i = 1; foreach($cpc_hour['price'] as $val){ if($i > 1){ echo ",";} echo $val; $i ++;}?>]
					}
				]
			};
