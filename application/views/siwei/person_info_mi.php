...
<form onSubmit="return chkForm();" action="?c=gonghai&m=order_update" method="post" class="form-horizontal" style="position:relative;">
...
<div class="span6 order_from">
	...
	<div class="control-group order_from">
	    ...
	    <?php  }else  if(!isset($order_edit_person_info)){ ?>
               ...
               <input type="hidden" value="<?php echo $info['pat_phone']; if(!empty($info['pat_phone1'])){echo "/" . $info['pat_phone1'];}?>" data-trigger="hover" data-placement="right" data-content="【香港区号为：00852】【固定电话格式：075512345678】【手机号码前面不加0】" data-original-title="号码输入规则" class="input-large popovers" name="patient_phone" id="patient_phone" />
		...
		<div class="control-group order_from">
			...
			<div class="controls">
			   ...
			   <?php foreach ($keshiurl_data as $keshiurl_data_temp){?>
			   	<option value="<?php echo $keshiurl_data_temp['id']?>"  <?php if(strcmp($keshiurl_data_temp['id'],$info['keshiurl_id']) == 0){?>selected="selected"<?php }?>><?php echo $keshiurl_data_temp['title']?>-<?php echo $keshiurl_data_temp['url']?></option>
			    <?php }?>
			   </select>
			</div>
		</div>
...
<div class="control-group order_from">
	<label class="control-label">商务通来源网址</label>
		<div class="controls">
		    <input type="text" name="laiyuanweb" id="laiyuanweb" value="" class="input-xlarge"  style="width:367px;"/>
		</div>
</div>
<div class="control-group order_from">
...
<?php endif;?>
	...
	$("#keshi_id").change(function(){
		...
		if($("#hidden_check_hos_id").val() != $("#hos_id").val() || $("#hidden_check_keshi_id").val() != $("#keshi_id").val() ||  $("#defult_patient_phone").val() != $("#patient_phone").val()){
												...
												4.变更咨询员会导致所有记时 限制的条件初始化
												 ...
												 if($("#admin_id_check").val() == $("#admin_id").val()){
													 if($("#yd_admin_id_check").val() == $("#admin_id_check").val() ){//当前咨询员自己更改时间
														...
														if($("#yd_sum_check").val()  == 3){
															if (Date.parse($("#order_hidden_time").val()) > Date.parse($("#order_time").val())) {
																$("#order_time_type_msg").html("预到时间必须大于:"+$("#order_hidden_time").val()+"");time_ok = 1;
															}
														}else{
															$("#order_time_type_msg").html("当前咨询剩余修改次数:"+(3-$("#yd_sum_check").val())+"次");time_ok = 1;
														}
													}
												 }
										 }
								    ...
								    if (Date.parse(date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate()) > Date.parse($("#order_time").val())) {
										...
										time_ok = 1;
								    	 ...
								    	 var time=$("#order_time").val();
										  var time1=new Date(time);
										  var time2=new Date();
										 var day = time2.getFullYear()+"-"+(time2.getMonth()+1)+"-"+time2.getDate();
										 if (Date.parse(day) > Date.parse(time)) {
											$("#order_time_type_msg").html("预到时间必须大于:"+day+"");time_ok = 1;
										 }else{
											  var end_time=time2.getTime()+30*24*60*60*1000;
											  if(time1.getTime() > end_time){
												  $("#order_time_type_msg").html("不能超过30天哦！从今天开始计算。");time_ok = 1;
