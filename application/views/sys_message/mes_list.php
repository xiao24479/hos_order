				...
				<!-- BEGIN PAGE CONTENT-->
					...
					<div class="row-fluid">
                                                ...
                                                <div class="span9" style="margin-top:50px;">
                                                    <label><a href="?c=sys_message&m=mes_add">新增公告</a></label>
                                                    <table width="100%">
                                                        <tr><td width="50">编号</td><td width="300">标题</td><td width="80">发布时间</td><td width="200">发布人</td><td width="60">操作</td></tr> 
                                                        <?php 
                                                        $i=0;
                                                        foreach($mes_list as $val){
                                                            $i++;
                                                            ?>
                                                        <tr <?php if(($i+1)%2==0){echo "style='background-color:#fff'";}?>><td width="50"><?=$i?></td><td width="300"><?=$val['message_title']?></td><td width="80"><?php echo date('Y-m-d',$val['message_time'])?></td><td width="200"><?=$val['message_user']?></td><td><a href="?c=sys_message&m=edit_mes&id=<?=$val['message_id']?>">编辑</a><a href="" style="color:#00a186;margin-left:10px; cursor: pointer;" onclick="order_window('?c=sys_message&m=message_content&id=<?=$val['message_id']?>')">查看</a></td></tr>
                                                        <?php }?>
                                                    </table>
