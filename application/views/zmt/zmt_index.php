    ...
    <body style="margin:0 auto;background-color: #f7f7f7;">
       ...
       <div id="content" style="position:absolute; width:100%; height:100%; margin-top:0px;"> 
           <div id="top" style="position:fixed; z-index: 999; background-color: #f7f7f7;height:150px;">
               <div id="nav_top" style="width:100%;height:15%;background-repeat:no-repeat;"><img style="position:fixed; z-index: 999;" src="static/img/nav.jpg" height="12%" width="100%" /><label style="position:fixed; z-index: 999;margin-left: 82%;margin-top:6px;">
                       ...
                       <img src="static/img/people.png" width="30px;"/><a onclick="" style="color:#fff;padding-left: 8px;"><?php echo $_COOKIE['l_admin_name'];?></a><a href="?c=index&m=logout" style="color:red;padding-left: 8px;"><img src="static/img/logout.png"/></a></label> </div>
           <form action="" method="get" class="date_form order_form" id="order_list_form"style="width:100%;">

<input type="hidden" value="order" name="c" />

<input type="hidden" value="zmt_index" name="m" /> 
           <div id="dy_search" style=" position:fixed;width:100%;height:10%;padding-top:10px;z-index: 3;margin-left: 50%;"> 
               
               <div style="position:fixed;padding-top:75px;margin-left: -600px;width:100%;height: 50px;line-height: 30px;font-family: '微软雅黑';color: #808080;font-size: 18px;background-color: #f7f7f7;">
                   <span style="margin-left: 30px;" onclick="order_window('?c=order&m=zmt_order_info&type=mi');"><img src="static/img/add_order.jpg"style="vertical-align:middle;height:30px; "/></span>
...
<div class="row-form" style="padding-top:42px;">
  ...
  if(!empty($order_list)){
    ...
    <div id="pat_<?php echo $item['order_id']; ?>">
		...
		<?php else:?>
			<?php if($rank_type == 5|| $rank_type == 1|| $_COOKIE['l_admin_action'] == 'all' || $item['is_come'] > 0):?>
				<?php if(in_array($item['hos_id'],$hos_auth) && $rank_type == 5 && $item['admin_id'] != $_COOKIE['l_admin_id']):?>
	...
	<td>
		...
		<?php 
		if(empty($item['zampo'])):
		if($rank_type == 5 || $rank_type == 3 || $_COOKIE['l_admin_action'] == 'all'):
		?>

		<?php 
		if((in_array(66, $admin_action)) || ($_COOKIE['l_admin_action'] == 'all') || ($rank_type == 5)):
		?>
			<?php if(in_array($item['hos_id'],$hos_auth)):?>
				<?php if($item['admin_id'] == $_COOKIE['l_admin_id'] || $_COOKIE['l_admin_id']== '239' || $_COOKIE['l_admin_id']== '71' || $_COOKIE['l_admin_action'] == 'all'):?>
					<a href="javascript:order_window('?c=order&m=zmt_order_info&type=mi&order_id=<?php
					echo $item['order_id'];
					...
					?>');" class="btn btn-info" onclick="_czc.push(['_trackEvent', '预约列表', '<?php echo $admin['name']; ?>', '编辑','','']);">编辑</a>
				<?php endif;?>
			<?php else:?>
				<a href="javascript:order_window('?c=order&m=zmt_order_info&type=mi&order_id=<?php
				...
				?>');" class="btn btn-info" onclick="_czc.push(['_trackEvent', '预约列表', '<?php echo $admin['name']; ?>', '<?php echo $this->lang->line('edit'); ?>','','']);">编辑</a>
		...
		?>
		<?php
		if($rank_type == 5 || $_COOKIE['l_admin_action'] == 'all'):
		...
		?>
			<?php if(in_array($item['hos_id'],$hos_auth)):?>
				<?php if($item['admin_id'] == $_COOKIE['l_admin_id'] || $_COOKIE['l_admin_id']== '239' || $_COOKIE['l_admin_id']== '71' || $_COOKIE['l_admin_action'] == 'all'):?>
					...
					?>');" class="btn btn-info" onclick="_czc.push(['_trackEvent', '预约列表', '<?php echo $admin['name']; ?>', '编辑','','']);">编辑中</a>
                                         <a href="#visit" role="button" class="btn btn-success" data-toggle="modal" onClick="ajax_remark(<?php echo $item['order_id']?>);_czc.push(['_trackEvent', '预约列表', '<?php echo $admin['name']; ?>', '<?php echo $this->lang->line('visit'); ?>','','']);"><?php echo $this->lang->line('visit'); ?></a>
				<?php endif;?>
			<?php else:?>
				<a href="javascript:order_window('?c=order&m=zmt_order_info&type=mi&order_id=<?php
				...
				?>');" class="btn btn-info" onclick="_czc.push(['_trackEvent', '预约列表', '<?php echo $admin['name']; ?>', '<?php echo $this->lang->line('edit'); ?>','','']);">编辑中</a>
                               <a href="#visit" role="button" class="btn btn-success" data-toggle="modal" onClick="ajax_remark(<?php echo $item['order_id']?>);_czc.push(['_trackEvent', '预约列表', '<?php echo $admin['name']; ?>', '<?php echo $this->lang->line('visit'); ?>','','']);"><?php echo $this->lang->line('visit'); ?></a> 
			<?php endif;?>
